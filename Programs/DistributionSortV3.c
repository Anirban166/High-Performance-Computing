...
// Example: mpicc -O3 DistributionSortV3.c -lm -o DistributionSortV3 && mpirun -np 2 -hostfile myhostfile.txt ./DistributionSortV3
#include <mpi.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) 
{
  ...
  // Declaring, allocating memory for and initializing local and global counters that holds the frequency of data for a rank and across all ranks respectively:
  unsigned int *localCounter = (unsigned int *)malloc(sizeof(unsigned int) * MAXVAL);  
  unsigned int *globalCounter = (unsigned int *)malloc(sizeof(unsigned int) * MAXVAL);
  ...
  for(int i = 0; i < localN; i++) 
  {
    localCounter[data[i]] += 1;
  }

  // Sending the local counters to rank 0 from other ranks, and then computing the global counter on rank 0 based on the received values:
  if(rank != 0) 
  {
    MPI_Send(localCounter, MAXVAL, MPI_UNSIGNED, 0, 2, MPI_COMM_WORLD);
  }
  if(!rank) 
  {
    for(int i = 0; i < size; i++) 
    {
      if(i != rank) 
      {
        MPI_Recv(localCounter, MAXVAL, MPI_UNSIGNED, i, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
      for(int i = 0; i < MAXVAL; i++) 
      {
        globalCounter[i] += localCounter[i];
      }
    }
    
    // Computing and assigning the ranges for each rank, adjusted based on the frequency distribution of the data:
    unsigned int distributionRatio = N / size;
    unsigned int rangeShift = distributionRatio;
    unsigned int rangeMin = 0, localCounter = 0, rankCounter = 0; 
    for(int j = 0; j < MAXVAL && rankCounter < size; j++) 
    {
      localCounter += globalCounter[j];
      if(localCounter >= rangeShift && rankCounter != size - 1) 
      {
        dataRange[rankCounter][0] = rangeMin;
        dataRange[rankCounter][1] = j;
        rangeMin = j;
        rangeShift += distributionRatio;
        rankCounter++;
      }
      else
      {

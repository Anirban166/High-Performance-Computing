...
// Execution: mpirun -np <processcount> -hostfile <hostfilename>.<hostfileextension> ./<executablename> <numberoflines> <dimensionality> <filename>
// Example: mpicc -O3 DistanceMatrix.c -lm -o DistanceMatrix && mpirun -np 2 -hostfile myhostfile.txt ./DistanceMatrix 100000 90 MSD_year_prediction_normalize_0_1_100k.txt
#include <mpi.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) 
{
  ...
  // Process command-line arguments: (N, Dimensionality and filename of the dataset; excluding block/tile size since its not applicable here)
  int N, DIM, blocksize;
  char inputFilename[500];
  if(argc != 4) 
  {
    fprintf(stderr, "\nPlease provide the following on the command line: N (number of lines in the file), dimensionality (number of coordinates per point), dataset filename.\n");
    MPI_Finalize();
    exit(0);
  }
  sscanf(argv[1], "%d", &N);
  ...
  // Initialize variables to hold the timings and make rank 0 collect the start time:
  ...
  for(int i = 0; i < localRowSize; i++)
  {
    for(int j = 0; j < N; j++)
    {
      ...
      for(int k = 0; k < DIM; k++) 
      {
        ...
        // printf("\nLocal Index for rank %d: %d", rank, localIndex);
  ...
  if(!rank) fprintf(stdout, "Global sum of all the distances computed locally by each rank: %f\n", globalSum);
  
  /* Uncomment this to verify the parallel distance matrix output (or to compare it with the sequential version), while using 2 ranks:
     mpicc -O3 DistanceMatrix.c -lm -o DistanceMatrix && mpirun -np 2 -hostfile myhostfile.txt ./DistanceMatrix 100 90 MSD_year_prediction_normalize_0_1_100k.txt

  // Writing the distance matrix elements to a file using this parallel version: (for comparison as required)
  int temp = 0, tempBuffer; // A variable to send and one to receive at, just for the sends and receives.
  if(!rank)
  {
     // Open local file in append+ mode to write the distance matrix elements to:
     FILE *fp;
     fp = fopen("distanceMatrixOutputParallel.txt", "a+"); // Using 'w' would overwrite :)
     for(int i = 0; i < localRowSize; i++)
     {
        for(int j = 0; j < N; j++)
          fprintf(fp, "%f ", distanceMatrix[i][j]);
        fprintf(fp, "\n");  
      } 
      fclose(fp); // fflush(fp) tends to be unnecessary, given that fclose() does a flush.
      MPI_Send(&temp, 1, MPI_INT, 1, 0, MPI_COMM_WORLD); // To avoid overlaps from other ranks
      // MPI_Recv(&tempBuffer, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  
  }
  if(rank == 1)
  {
      MPI_Recv(&tempBuffer, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // will wait
      FILE *fp;
      fp = fopen("distanceMatrixOutputParallel.txt", "a+");
      for(int i = 0; i < localRowSize; i++)
      {
        for(int j = 0; j < N; j++)
          fprintf(fp, "%f ", distanceMatrix[i][j]);
